...
SELECT
    tipo_sinistro,
    status,
    -- bucket de tempo de espera desde a abertura (idade da fila)
    CASE
    ...
    ROUND(SUM(valor_estimado), 2)                                AS valor_exposto_total,
    ROUND(AVG(valor_estimado), 2)                                AS valor_medio,
    -- SLA: há quantas horas o sinistro mais antigo da fila está parado
    ROUND(
        (unix_timestamp(current_timestamp()) - unix_timestamp(MIN(data_hora_abertura)))
        / 3600.0
    , 1)                                                         AS horas_espera_mais_antigo,
    -- pressão de entrada: quantos caíram na fila na última janela de CDC (15 min)
    SUM(CASE WHEN dt_insercao >= current_timestamp() - INTERVAL '15' MINUTE
             THEN 1 ELSE 0 END)                                  AS entradas_ultimos_15min,
...
GROUP BY
    ...
    CASE
        ...
        ELSE '24h+'
    END
ORDER BY qtd_sinistros DESC, valor_exposto_total DESC;